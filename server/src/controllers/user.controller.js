...
const login = async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.status(401).json({ success, "msg": "No User Found" });

        // Match entered password against the signup password
        let checkPass = await user.checkPassword(password);

        if (!checkPass) {
            return res.status(400).json({ success, "msg": "Please login using correct credentials." });
        }
...
const updatePassword = async (req, res) => {
    ...
    try {
        ...
        await user.save();
        success = true;
        res.status(200).json({ success, user: user._id, msg: "Password updated successfully"});
        
    } catch (error) {
        res.status(500).json({success, "error-log": error, "msg": "Password updation failed."});
    }
};
